...
import { createAzureDevOpsClient } from "../../src/clients/azureDevOps";
...
function harness() {
  ...
  const ado = createAzureDevOpsClient(config, silentLogger);
  ...
  const app = createApp({ config, queue, logger: silentLogger });
  const worker = createWorker({
    queue,
    maxRetries: config.worker.maxRetries,
    logger: silentLogger,
    syncDeps: { config, connection, ado, plane, store, logger: silentLogger },
  });
...
describe("POST /webhooks/azure-devops", () => {
  ...
  it("enqueues then syncs through create -> stale-skip -> update", async () => {
    ...
    let sync = await store.getWorkItemSync("myorg", "myproject", 42);
    expect(sync?.plane_issue_id).toBe("issue-1");
    expect(sync?.last_ado_rev).toBe(3);
    ...
    expect((await store.getWorkItemSync("myorg", "myproject", 42))?.last_ado_rev).toBe(3);
    ...
    sync = await store.getWorkItemSync("myorg", "myproject", 42);
    expect(sync?.last_ado_rev).toBe(6);
  });
});
